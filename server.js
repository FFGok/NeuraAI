...
const DATA_DIR = process.env.DATA_DIR || __dirname;const fetch = (...args) =>
  import("node-fetch").then(({ default: fetch }) => fetch(...args));
...
function sistemPromptOlustur(konusmaModu){
  ...
  if(konusmaModu === "profesor"){
    return temel +
      " Şu an Profesör Modundasın. Ancak bu mod Pro gerektirir. Kullanıcı bu modda cevap isterse kibarca Profesör Modu için Pro gerektiğini söyle.";
  }
