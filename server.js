...
function modelProfili(tip){
  ...
  if(t === "yaratici"){
    return {
      ad:"Yaratıcı",
      model: process.env.OPENROUTER_CREATIVE_MODEL || "openai/gpt-4.1-mini",
      maxKisa:1200,
      maxUzun:2600,
  ...
  if(t === "kod"){
    return {
      ad:"Kod",
      model: process.env.OPENROUTER_CODE_MODEL || "openai/gpt-4.1-mini",
      maxKisa:1800,
      maxUzun:4200,
  ...
  return {
    ad:"Akıllı",
    model: process.env.OPENROUTER_SMART_MODEL || "openai/gpt-4.1-mini",
    maxKisa:1200,
    maxUzun:3200,
